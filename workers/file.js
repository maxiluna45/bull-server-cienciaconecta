...
export const ActualizarDocumentsWotker = async (job, done) => {
  try {
    ...
    if (id_archivo_pdf || id_carpeta_campo || id_autorizacion_imagen || id_informe_trabajo) {
      proyecto.registroPedagogico = id_archivo_pdf ? `https://drive.google.com/file/d/${id_archivo_pdf}/preview` : proyecto.registroPedagogico;
      proyecto.carpetaCampo = id_carpeta_campo ? `https://drive.google.com/file/d/${id_carpeta_campo}/preview` : proyecto.carpetaCampo;
      proyecto.informeTrabajo = id_informe_trabajo ? `https://drive.google.com/file/d/${id_informe_trabajo}/preview` : proyecto.informeTrabajo;
      proyecto.autorizacionImagen = id_autorizacion_imagen ? `https://drive.google.com/file/d/${id_autorizacion_imagen}/preview` : proyecto.autorizacionImagen;
      await proyecto.save();
      done();
    } else {
      done(error);
